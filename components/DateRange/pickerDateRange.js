...
function pickerDateRange(inputId, options) {
    ...
    this.periodObj[__method.mOpts.aRecent60Days] = 60;
    this.periodObj[__method.mOpts.aRecent90Days] = 90;
    // 记录初始默认时间
    this.startDefDate = '';
    // 随机ID后缀
    ...
    $('#' + this.calendarId).css('z-index', 100000);
    // 初始化目标地址的元素
...
pickerDateRange.prototype.init = function(isCompare) {
    ...
    for (var property in __method.periodObj) {
        if ($('#' + property).length > 0) {
            ...
            var startTime = new Date(this.mOpts.startDate).getTime();
            var endTime = new Date(this.mOpts.endDate).getTime();
            var timeDiff = endTime - startTime;
            var dayDiff = timeDiff / (1000 * 24 * 60 * 60);
            var date = new Date();
            var todayStr = date.getFullYear() + '-';
            if ((date.getMonth() + 1) < 10) {
                todayStr += "0";
            }
            todayStr += (date.getMonth() + 1) + "-";
            if (date.getDate() < 10) {
                todayStr += "0";
            }
            todayStr += date.getDate();
            if (dayDiff == __method.periodObj[$('#' + property).attr('id')] && todayStr == this.mOpts.endDate) {
                $('#' + property).parent().addClass(cla);
            }

            $('#' + property).unbind('click');
            // console.log(__method.inputId,property,'=-=-=');
            $('#' + property).bind('click', function() {

                // console.log(__method.inputId);
                //处理点击样式
                var cla = __method.mOpts.theme == 'ta' ? 'active' : 'a';
...
pickerDateRange.prototype.getSpecialPeriod = function(period) {
    var __method = this;
    //  var date = new Date();
    var date = this.mOpts.stopTime ? new Date(this.mOpts.stopTime) : new Date();
...
pickerDateRange.prototype.selectDate = function(ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        if (true == this.mOpts.singleCompare || true == this.mOpts.isSingleDay) {
            ...
            $('#' + this.dateInput).val(ymdFormat); //modify by nikai
        }
...
pickerDateRange.prototype.close = function(btnSubmit) {
    ...
    if (btnSubmit) {
        ...
        if ($("#" + this.calendarId + ':visible').length > 0) {
            this.mOpts.close();
        }
    }
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    for (var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if (d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '1';
        } else if ((this.mOpts.stopTime && d.getTime() > this.mOpts.stopTime) || (this.mOpts.stopToday == true && d.getTime() > today.getTime()) || d.getTime() < __method.mOpts.minValidDate * 1000) { // 当前时间之后的日期，或者开启统计之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期
            deviation = '0';
